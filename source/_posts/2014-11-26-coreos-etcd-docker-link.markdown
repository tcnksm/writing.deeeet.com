...
このコンテナは特定のポート（10000）のトラフィックを与えられた引数のホストにforwardする．CoreOSのDockerコンテナからは`172.17.42.1:4001`でそのetcdにアクセスできる．引数にそれを与えることで，`10000`portへのトラフィックはすべてetcdにforwardされるようになる．
